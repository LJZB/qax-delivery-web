...
test.describe("QAXadmin órdenes API + UI", () => {
  test("crea una orden por API, valida estadísticas y cambia estado desde UI", async ({
    page,
    request,
  }, testInfo) => {
    ...
    const uniqueId =
      `${testInfo.project.name}-${Date.now()}-${testInfo.workerIndex}`.replace(
        /[^a-zA-Z0-9-]/g,
        "-",
      );
    const order = {
    ...
    await test.step("Preparar orden y validar estadísticas desde API REST", async () => {
      ...
      // Validación backend: la orden propia queda pendiente y el contador global no retrocede.
      // En paralelo, otros workers pueden crear órdenes entre statsBefore y statsAfter.
      expect(backendOrder).toMatchObject({ ...order, estado: "Pendiente" });
      expect(statsAfter.totalOrdenes).toBeGreaterThanOrEqual(statsBefore.totalOrdenes + 1);
      Object.assign(order, {
        id: createdOrder.id,
